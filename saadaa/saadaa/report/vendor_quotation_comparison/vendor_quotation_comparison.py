...
def execute(filters=None):
	...
	vendor_quotation_data = get_data(filters)

	data, chart_data = prepare_data(vendor_quotation_data, filters)
	message = get_message()
...
def get_data(filters):
	sq = frappe.qb.DocType("Vendor Quotation")
	sq_item = frappe.qb.DocType("Vendor Quotation Item")
	...
	query = (
		...
		.select(
			...
			sq_item.uom,
			sq_item.stock_uom,
			sq_item.request_for_quotations,
			(sq_item.lead_time_days * 1440).as_("lead_time_minutes"),
	...
	if filters.get("vendor_quotation"):
	...
	if not filters.get("include_expired"):
		query = query.where(sq.vq_status != "Expired")
...
def prepare_data(supplier_quotation_data, filters):
	...
	for data in supplier_quotation_data:
		...
		row = {
			...
			"request_for_quotations": data.get("request_for_quotations"),
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"fieldname": "quotation",
			"label": _("Vendor Quotation"),
			"fieldtype": "Link",
			"options": "Vendor Quotation",
			"width": 200,
		},
		{"fieldname": "valid_till", "label": _("Valid Till"), "fieldtype": "Date", "width": 100},
		{
			"fieldname": "lead_time_minutes",
			"label": _("Lead Time (minutes)"),
			"fieldtype": "Int",
			"width": 100,
		},
		{
			"fieldname": "request_for_quotations",
			"label": _("Request For Quotations"),
			"fieldtype": "Link",
			"options": "Request For Quotations",
			"width": 150,
		},
	]
