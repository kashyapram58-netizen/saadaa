...
frappe.query_reports["Vendor Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = frappe.query_report.get_filter_value("vendor_quotation");
				if (quote != "") {
					return {
		...
		{
			fieldtype: "MultiSelectList",
			label: __("Vendor Quotation"),
			fieldname: "vendor_quotation",
			options: "Vendor Quotation",
			default: "",
			get_data: function (txt) {
				return frappe.db.get_link_options("Vendor Quotation", txt, { docstatus: ["<", 2] });
			},
		},
		{
			fieldtype: "Link",
			label: __("Request For Quotations"),
	...
	onload: (report) => {
		...
		report.page.add_inner_button(
			...
			() => {
				let reporter = frappe.query_reports["Vendor Quotation Comparison"];
	...
	make_default_supplier_dialog: (report) => {
		...
		let dialog = new frappe.ui.Dialog({
			title: __("Select Default Vendor"),
			fields: [
				{
					reqd: 1,
					label: "Vendor",
		...
		dialog.set_primary_action(__("Set Default Vendor"), () => {
			...
			if (values) {
				...
				frappe.call({
					...
					freeze: true,
					callback: (r) => {
						frappe.msgprint(__("Successfully Set Vendor"));
						dialog.hide();
					},
				});
			}
